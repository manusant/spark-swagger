...
public class IgnoreSpec {
        ...
        if (this.ignoreTypes != null) {
            if (field.getType().isArray() || Collection.class.isAssignableFrom(field.getType())) {
                ...
                if (type instanceof ParameterizedType) {
                    if (ignoreParameterizedType((ParameterizedType) type, this.ignoreTypes)) return true;
                }
            }
    ...
    private boolean ignoreParameterizedType(ParameterizedType type, List<Class<?>> ignoreTypes) {
        for (Type paramType : type.getActualTypeArguments()) {
            if (ignoreTypes.stream().map(Class::getTypeName).anyMatch(typeName -> typeName.equalsIgnoreCase(paramType.getTypeName()))) {
                return true;
            }
        }
        return false;
